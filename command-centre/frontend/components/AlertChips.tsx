...
export default function AlertChips({
  ...
  return (
    <div className="pointer-events-auto absolute right-4 top-16 z-20 flex w-[19rem] max-w-[calc(100vw-2rem)] flex-col gap-2">
      {crossHubs.map((h, i) => {
        ...
        return (
          <button
            ...
            className={`glass w-full border-l-2 p-3 text-left transition hover:brightness-125 ${
      ...
      {topScam && (
        <button
          ...
          className="glass w-full border-l-2 !border-l-red-500/70 p-3 text-left transition hover:brightness-125"
        >
          <div className="flex items-center gap-1.5">
      ...
      {onOpenAll && (
        <button
          onClick={onOpenAll}
          className="self-end rounded-full px-2 py-0.5 text-[10px] text-zinc-500 transition hover:text-zinc-300"
        >
          view all alerts →
        </button>
      )}
