...
export default function Page() {
  ...
  /** Provenance of the briefing on screen, as one dot. Three states, because
   *  "retained real analysis" and "computed from counts" are not the same
   *  degradation and shouldn't look alike:
   *    emerald — generated live for this data
   *    blue    — a real model briefing, retained (memory or Mongo) because the
   *              providers were unreachable; may predate the newest detections
   *    amber   — no provider AND no cached briefing, so this is computed
   *              directly from the event stream rather than written
   *  aria-label + title carry the same information, so colour is never the
   *  only channel. */
  const summaryProvenance = !aiSummaries
    ? null
    : aiSummaries.cached
      ? {
          dot: "bg-sky-400 shadow-[0_0_6px_2px_rgba(56,189,248,0.65)]",
          label: "Retained AI briefing",
          title:
            `Live synthesis unreachable — showing the last briefing generated by ` +
            `${aiSummaries.engine}${aiSummaries.generated_at ? ` at ${aiSummaries.generated_at}` : ""}. ` +
            `It may predate the newest detections.`,
        }
      : aiSummaries.engine?.includes("fallback")
        ? {
            dot: "bg-amber-400 shadow-[0_0_6px_2px_rgba(251,191,36,0.6)]",
            label: "Computed summary",
            title:
              "No AI provider reachable and no cached briefing — this text is computed " +
              "directly from the live event stream, not written by a model.",
          }
        : {
            dot: "bg-emerald-400 shadow-[0_0_6px_2px_rgba(52,211,153,0.6)]",
            label: "Live AI briefing",
            title: `Generated live by ${aiSummaries.engine}`,
          };
  ...
  return (
    <main className="relative h-dvh w-screen select-none overflow-hidden bg-zinc-950">
      ...
      {activeTab === "research" && (
        <div className="absolute inset-0 z-40 bg-black/65 backdrop-blur-md flex flex-col items-center justify-center px-6 pb-6 pt-28 pointer-events-auto">
            ...
            <div className="gsap-panel flex-1 min-w-0 max-h-[90vh] overflow-y-auto bg-zinc-900 border border-white/10 shadow-2xl" style={{ opacity: 0 }}>
              {selectedModule ? (
                ...
                <div className="p-6 flex flex-col gap-6 h-full">
                  ...
                  <div className="bg-white/5 border border-white/10 p-5 flex-1 relative min-h-[300px] overflow-y-auto scroll-thin whitespace-pre-line">
                    <div className="text-xs font-medium text-zinc-300 mb-3 flex justify-between items-center">
                      AI Intelligence Overview
                      {summaryProvenance && <span aria-label={summaryProvenance.label} title={summaryProvenance.title} className={`h-2 w-2 shrink-0 rounded-full ${summaryProvenance.dot}`} />}
                    </div>
                    {aiSummaries ? (
